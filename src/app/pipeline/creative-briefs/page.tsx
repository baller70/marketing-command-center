...
const statusColors: Record<string, string> = { submitted: "bg-[var(--bg-card)] text-[var(--text-primary)]", acknowledged: "bg-[var(--bg-card)] text-[var(--text-primary)]", in_production: "bg-[var(--bg-card)] text-[var(--text-primary)]", delivered: "bg-[var(--bg-card)] text-[var(--text-primary)]", cancelled: "bg-[var(--bg-card)] text-[var(--text-primary)]" }
  ...
  return (
    <div className="p-6 max-w-[1400px] mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs text-[var(--text-muted)] ml-auto">{briefs.length} briefs</span>
      </div>

      {showForm && (
        <div className="rounded-xl bg-[var(--bg-primary)] border border-[var(--border)]/30 p-6 space-y-4">
          <h3 className="text-sm font-semibold text-[var(--text-primary)]">New Creative Brief → Content Division</h3>
          <div className="grid grid-cols-3 gap-4">
            ...
            <div><label className="block text-xs text-[var(--text-secondary)] mb-1">Campaign Name</label>
              <input value={form.campaignName} onChange={e => setForm({...form, campaignName: e.target.value})} className="w-full bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg px-3 py-2 text-sm text-[var(--text-secondary)] outline-none" /></div>
            <div><label className="block text-xs text-[var(--text-secondary)] mb-1">Goal</label>
              <select value={form.campaignGoal} onChange={e => setForm({...form, campaignGoal: e.target.value})} className="w-full bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg px-3 py-2 text-sm text-[var(--text-secondary)] outline-none">
                {GOALS.map(g => <option key={g} value={g}>{g.replace("_", " ")}</option>)}
              </select></div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-xs text-[var(--text-secondary)] mb-1">Key Message</label>
              <textarea value={form.keyMessage} onChange={e => setForm({...form, keyMessage: e.target.value})} rows={2} className="w-full bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg px-3 py-2 text-sm text-[var(--text-secondary)] outline-none resize-none" /></div>
            <div><label className="block text-xs text-[var(--text-secondary)] mb-1">CTA</label>
              <input value={form.cta} onChange={e => setForm({...form, cta: e.target.value})} placeholder="e.g., Book your free trial" className="w-full bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg px-3 py-2 text-sm text-[var(--text-secondary)] outline-none placeholder:text-[var(--text-muted)]" /></div>
          </div>
          <div className="grid grid-cols-3 gap-4">
            <div><label className="block text-xs text-[var(--text-secondary)] mb-1">Messaging Lane</label>
              <input value={form.messagingLane} onChange={e => setForm({...form, messagingLane: e.target.value})} className="w-full bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg px-3 py-2 text-sm text-[var(--text-secondary)] outline-none" /></div>
            <div><label className="block text-xs text-[var(--text-secondary)] mb-1">Target Audience</label>
              <input value={form.targetAudience} onChange={e => setForm({...form, targetAudience: e.target.value})} className="w-full bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg px-3 py-2 text-sm text-[var(--text-secondary)] outline-none" /></div>
            <div className="flex gap-2">
              <div className="flex-1"><label className="block text-xs text-[var(--text-secondary)] mb-1">Deadline</label>
                <input type="date" value={form.deadline} onChange={e => setForm({...form, deadline: e.target.value})} className="w-full bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg px-3 py-2 text-sm text-[var(--text-secondary)] outline-none" /></div>
              <div><label className="block text-xs text-[var(--text-secondary)] mb-1">Priority</label>
          ...
          <div>
            ...
            {form.assetsNeeded.map((asset, idx) => (
              <div key={idx} className="grid grid-cols-4 gap-2 mb-2">
                <select value={asset.type} onChange={e => updateAsset(idx, "type", e.target.value)} className="bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg px-2 py-1.5 text-xs text-[var(--text-secondary)] outline-none">
                  {ASSET_TYPES.map(t => <option key={t}>{t}</option>)}
                </select>
                <input type="number" value={asset.quantity} onChange={e => updateAsset(idx, "quantity", parseInt(e.target.value))} className="bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg px-2 py-1.5 text-xs text-[var(--text-secondary)] outline-none placeholder:text-[var(--text-muted)]" placeholder="Qty" />
                <select value={asset.platform} onChange={e => updateAsset(idx, "platform", e.target.value)} className="bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg px-2 py-1.5 text-xs text-[var(--text-secondary)] outline-none">
                  {PLATFORMS.map(p => <option key={p}>{p}</option>)}
                </select>
                <input value={asset.requirements} onChange={e => updateAsset(idx, "requirements", e.target.value)} placeholder="Specific requirements..." className="bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg px-2 py-1.5 text-xs text-[var(--text-secondary)] outline-none placeholder:text-[var(--text-muted)]" />
              </div>
            ))}
          </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl bg-[var(--bg-primary)] border border-[var(--border)] p-12 text-center">
          ...
          <p className="text-[var(--text-muted)]">No creative briefs yet</p>
          <p className="text-xs text-[var(--text-muted)] mt-1">Submit briefs to request assets from the Content Division</p>
        </div>
      ) : (
        <div className="space-y-3">
          {briefs.map(b => (
            <div key={b.id} className="rounded-xl bg-[var(--bg-primary)] border border-[var(--border)] p-4 hover:border-[var(--border)] transition-colors">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2 flex-wrap">
                    <span className="text-sm font-semibold text-[var(--text-primary)]">{b.campaignName}</span>
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${statusColors[b.status]}`}>{b.status.replace("_", " ")}</span>
                    <span className="px-2 py-0.5 rounded-full bg-[var(--bg-secondary)] text-xs text-[var(--text-secondary)]">{b.brand}</span>
                    <span className="px-2 py-0.5 rounded-full bg-[var(--bg-secondary)] text-xs text-[var(--text-secondary)]">{b.priority}</span>
                  </div>
                  <p className="text-xs text-[var(--text-secondary)] italic mb-1">&ldquo;{b.keyMessage}&rdquo;</p>
                  ...
                  {(b.assetsNeeded as Brief["assetsNeeded"])?.length > 0 && (
                    <div className="flex gap-1.5 mt-2 flex-wrap">
                      {(b.assetsNeeded as Brief["assetsNeeded"]).map((a, i) => (
                        <span key={i} className="px-2 py-0.5 rounded-full bg-[var(--bg-secondary)] text-[10px] text-[var(--text-secondary)] border border-[var(--border)]">
